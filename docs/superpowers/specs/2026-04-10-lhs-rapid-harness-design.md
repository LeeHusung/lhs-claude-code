...
3. `evaluation-criteria.md`의 채점 기준에 따라 검증:
   ...
   - 반응형 동작 확인 (요구 시)
4. 항목별 점수 산출 → 종합 점수
5. 80점 이상: PASS → 최종 결과 보고
6. 80점 미만: FAIL → 구체적 수정 사항을 `claude-progress.txt`에 기록 → Generator 재실행 지시
...
## 합격 기준
- 80점 이상: PASS → Deliver 단계로 진행
- 80점 미만: FAIL → 구체적 피드백과 함께 Generator 재실행
...
5. **자동 루프**: Evaluator 점수 < 80 → claude-progress.txt에 피드백 기록 → Generator 프롬프트를 다시 읽고 수정 구현 → 재평가 (최대 3회)
